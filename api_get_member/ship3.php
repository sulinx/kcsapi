...
try {
    ...
    $stmt = $conn->prepare("SELECT api_deck_port,api_ship FROM userdata where uid = $uid1");
    $stmt->execute();
    // 设置结果集为关联数组    
	$req = new req();	
	...
	$req->api_data->api_deck_data=$api_deck_port;	
	$req->api_data->api_ship_data=$api_ship;			
	$dsn = null;
    }
...
try {
	...
	$req->api_data->api_slot_data=$unsetslot;
